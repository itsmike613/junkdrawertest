...
function findItem() {
	...
	if (emptyIndex === -1) {
		infoContent.innerHTML =
			"<span style='color:var(--danger)'>Drawer is full!</span>";
		return;
	}
...
function updateStats() {
	...
	let grade = "F - Empty Pit";

	if (val >= 150) grade = "S- Antiques Roadshow";
	else if (val >= 75) grade = "A - Buried Treasure";
	else if (val >= 25) grade = "B - Dad's Secret Stash";
	else if (val >= 5) grade = "C - Misc. Chaos";
	else if (val > 0) grade = "D - Dust Bunnies";
	document.getElementById("stat-total").innerText = stats.total;
	document.getElementById(
		"stat-unique"
	).innerText = `${stats.unique.size}/${itemPool.length}`;
